...
import kfp
from kfp import dsl
from kfp.dsl import *  # noqa: F403


@dsl.component(base_image="python:3.9", packages_to_install=["requests", "beautifulsoup4"])
        ...
        try:
            ...
            for item in items:
                if item["type"] == "file" and (
                    ...
                    files.append(
                        {"path": item["path"], "content": content, "file_name": item["name"]}
                    )
                elif item["type"] == "dir":
...
@dsl.component(base_image="python:3.9", packages_to_install=["requests"])
...
def chunk_and_embed(
    ...
    with open(github_data.path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            content = re.sub(
                r"^\s*[+\-]{3,}.*?[+\-]{3,}\s*", "", content, flags=re.DOTALL | re.MULTILINE
            )
            ...
            print(
                f"File: {file_data['path']} -> {len(chunks)} chunks (avg: {sum(len(c) for c in chunks) / len(chunks):.0f} chars)"
            )
...
@dsl.component(base_image="python:3.9", packages_to_install=["pymilvus", "numpy"])
def store_milvus(
    embedded_data: dsl.Input[dsl.Dataset], milvus_host: str, milvus_port: str, collection_name: str
):
...
@dsl.pipeline(name="github-rag", description="RAG pipeline for processing GitHub documentation")
...
if __name__ == "__main__":
    ...
    kfp.compiler.Compiler().compile(
        pipeline_func=github_rag_pipeline, package_path="github_rag_pipeline.yaml"
    )
