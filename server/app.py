...
async def stream_llm_response(
    payload: Dict[str, Any], websocket, citations_collector: List[str] = None
) -> None:
                    ...
                    try:
                        ...
                        # Handle regular content
                            ...
                            await websocket.send(
                                json.dumps({"type": "content", "content": delta["content"]})
                            )
...
async def handle_tool_follow_up(
    ...
    try:
        ...
        await websocket.send(
            json.dumps({"type": "error", "content": f"Tool follow-up failed: {e}"})
        )
...
async def handle_chat(message: str, websocket) -> None:
    ...
    try:
        ...
        if citations_collector:
            ...
            await websocket.send(json.dumps({"type": "citations", "citations": unique_citations}))
    ...
    try:
        ...
        async for message in websocket:
            try:
                ...
                await websocket.send(
                    json.dumps({"type": "error", "content": f"Message processing failed: {e}"})
                )
