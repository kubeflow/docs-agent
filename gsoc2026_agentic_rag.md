...
12. **Continuous Evals & Quality Assurance:** We must implement an offline and online evaluation framework (e.g., RAGAS, LangSmith, or TruLens) to continuously measure context precision, answer recall, and factual groundedness. This ensures model drifts or bad documentation updates do not silently degrade the quality of responses over time.
13. **Data Isolation & Multi-Tenancy (RBAC for Indices):** We could explore physically or logically isolating Vector Collections. The "Documentation" collection and the "Release Code" collection might have separated RBAC and indexing to prevent "noisy neighbor" semantic overlap. If we expand to multi-tenant capabilities (e.g., separate Kubeflow Sub-projects), namespace-level vector isolation could prevent cross-contamination.
14. **Cost Monitoring & OCI Alerting:** Operating LLMs and Vector DBs at scale can incur unpredictable costs. The architecture must include strict cost-monitoring dashboards (e.g., integrating with Kubecost or native OCI billing metrics). We will configure automated OCI budget alerts to trigger Webhooks (e.g., PagerDuty/Slack) if daily LLM inference or vector storage costs exceed predefined thresholds, allowing maintainers to defensively scale down or enforce stricter rate limits.
15. **OCI Deployment & Platform WG Collaboration:** The entire stack will be primarily deployed and dogfooded on our Oracle Cloud Infrastructure (OCI) community cluster. To achieve this, the project requires tight collaboration with the **Kubeflow Platform Working Group (WG)**. This partnership will ensure that the underlying Kubeflow foundations (KServe, KFP, Istio) are stably provisioned on OCI, allowing the `docs-agent` stack to deploy seamlessly on top as a reference architecture.
16. **Environment Portability & Configurable Endpoints:** The architecture must be highly configurable via environment variables (e.g., LLM endpoints, DB connection strings, tool hosts). While our primary live deployment will be hosted on OCI and should leverage managed cloud-native backends where possible for resilience, the system must remain fully decoupled. This leaves room for a "bring your own" (BYO) approach if community members want to host the entire solution entirely on their own local Kubernetes cluster using open-source backends.
17. **User Logins, Rate Limiting, & Adaptive Quotas:** The system's exploratory target goal is supporting up to 1,000 concurrent users. To manage this safely and prevent budget exhaustion, we will require login for both the **Development MCP** and the **Website Frontend**. This login gateway enforces strict rate limiting and daily token quotas via middleware (e.g., Litellm), gathers valuable telemetry on Kubeflow platform usage patterns, and allows us to invite users to the Kubeflow mailing list—turning casual users into engaged community members. As a future state, we aim to provide additional tiered API credits to active maintainers, GSoC students, and core contributors. *Note: As part of the architectural design phase, the community will need to evaluate and agree upon the specific tech stack for managing user storage, authentication, and credit/quota logic.*
